...
function OrderForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const warehouseIds = warehousesResponse.data.map(
        (warehouse) => warehouse.id
      );

      const restaurantResponse = await axios.get(
        `http://localhost:8000/api/restaurants/${selectedRestaurant}/`
      );
      ...
      for (let i = 0; i < warehouseIds.length; i++) {
        const inventoryResponse = await axios.get(
          `http://localhost:8000/api/inventory/?warehouse=${warehouseIds[i]}`
        );
        const chosenItemInventory = inventoryResponse.data.filter(
          (item) => item.sku === selectedItem && item.quantity >= quantity
        );
        if (chosenItemInventory.length > 0) {
          const warehouseResponse = await axios.get(
            `http://localhost:8000/api/warehouses/${warehouseIds[i]}/`
          );
          ...
          closestWarehouses.push({
            ...
            inventory_id:
              chosenItemInventory[0].id || chosenItemInventory[0].inventory_id,
          });
        }
      }
  ...
  async function handleUpdateInventory(warehouse) {
    try {
      ...
      if (updatedQuantity < 0) {
        alert(
          'Unable to subtract the selected quantity from the warehouse inventory as it exceeds the available quantity.'
        );
        return;
      }
      ...
      const response = await axios.patch(
        `http://localhost:8000/api/inventory/${warehouse.inventory_id}/`,
        {
        ...
        <div>
          ...
          <ul>
            {closestWarehouses.map((warehouse) => (
              <li key={warehouse.id}>
                {warehouse.name} - Distance: {warehouse.distance.toFixed(2)},
                Quantity: {warehouse.quantity}, SKU: {warehouse.sku}, Item Name:{' '}
                {warehouse.item_name}
                <button onClick={() => handleUpdateInventory(warehouse)}>
                  Update Inventory
                </button>
              </li>
            ))}
